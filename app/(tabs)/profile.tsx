...
export default function ProfileScreen() {
  ...
  const { isLoading: authLoading, isMember } = useAuth();
  ...
  return (
    <Screen header={<View style={styles.header}><Text variant="headerTitle" accessibilityRole="header">마이페이지</Text>{isMember ? <Pressable accessibilityRole="button" accessibilityLabel="프로필 설정" onPress={() => router.push('/profile-settings' as never)} style={styles.profileButton}>{profileImageUri ? <Image source={{ uri: profileImageUri }} style={styles.profileImage} accessibilityLabel="프로필 사진" /> : <Icon name="person.crop.circle" size={24} color={colors.espresso} />}</Pressable> : <Pressable accessibilityRole="button" accessibilityLabel="로그인 또는 회원가입" onPress={() => router.push('/auth' as never)} style={styles.signInButton}><Icon name="person.crop.circle" size={20} color={colors.espresso} /><Text variant="caption">로그인</Text></Pressable>}</View>} contentContainerStyle={styles.screen}>
      ...
      {!authLoading && !isMember ? <Pressable accessibilityRole="button" accessibilityLabel="계정 연결. Google로 로그인" onPress={() => router.push('/auth' as never)} style={({ pressed }) => [styles.accountPrompt, pressed && styles.pressed]}><View style={styles.accountPromptIcon}><Icon name="person.crop.circle" size={21} color={colors.espresso} /></View><View style={styles.flex}><Text variant="title3">계정을 연결해보세요</Text><Text variant="caption" color={colors.neutral600}>Google 로그인으로 봉투 사진을 안전하게 보관해요.</Text></View><Icon name="chevron.right" size={17} color={colors.neutral400} /></Pressable> : null}
      <View style={styles.menu}>{isMember ? <MenuRow icon="person.crop.circle" title="계정 관리" body="로그인, 탈퇴 및 약관을 관리해요" onPress={() => router.push('/account' as never)} /> : null}<MenuRow icon="gearshape.fill" title="앱 설정" body="진동, 소리, 저장 및 앱 정보" onPress={() => router.push('/settings')} /></View>
    </Screen>
  );
}
...
const styles = StyleSheet.create({ screen: { gap: spacing.section }, flex: { flex: 1 }, header: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, profileButton: { width: 48, height: 48, borderRadius: 24, overflow: 'hidden', alignItems: 'center', justifyContent: 'center' }, signInButton: { minWidth: 72, height: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4 }, profileImage: { width: '100%', height: '100%' }, hero: { minHeight: 270, justifyContent: 'center', alignItems: 'flex-start', gap: spacing.compact, padding: spacing.section, borderRadius: radius.xl, backgroundColor: colors.warmBeige }, heroIcon: { width: 62, height: 62, borderRadius: 22, backgroundColor: colors.white, alignItems: 'center', justifyContent: 'center', marginBottom: spacing.compact }, more: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingTop: spacing.compact }, stats: { flexDirection: 'row', gap: spacing.compact }, stat: { flex: 1, minHeight: 108, alignItems: 'center', justifyContent: 'center', gap: spacing.compact, paddingHorizontal: spacing.xs, borderRadius: radius.large, backgroundColor: colors.white, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.neutral200 }, statValue: { flexDirection: 'row', alignItems: 'center', gap: 4 }, accountPrompt: { minHeight: 76, flexDirection: 'row', alignItems: 'center', gap: spacing.small, padding: spacing.small, borderRadius: radius.large, backgroundColor: colors.white, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.neutral200 }, accountPromptIcon: { width: 44, height: 44, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.creamDeep }, menu: { backgroundColor: colors.white, borderRadius: radius.large, overflow: 'hidden', borderWidth: StyleSheet.hairlineWidth, borderColor: colors.neutral200 }, menuRow: { minHeight: 88, flexDirection: 'row', alignItems: 'center', gap: spacing.small, padding: spacing.small }, menuIcon: { width: 52, height: 52, borderRadius: 18, backgroundColor: colors.creamDeep, alignItems: 'center', justifyContent: 'center' }, pressed: { opacity: 0.7, transform: [{ scale: 0.98 }] } });
