  ...
  return (
    <Screen header={<View style={styles.header}><Text variant="title1" accessibilityRole="header">프로필</Text><Pressable accessibilityRole="button" accessibilityLabel="프로필 설정" onPress={() => router.push('/profile-settings' as never)} style={styles.profileButton}>{profileImageUri ? <Image source={{ uri: profileImageUri }} style={styles.profileImage} accessibilityLabel="프로필 사진" /> : <Icon name="person.crop.circle" size={30} color={colors.espresso} />}</Pressable></View>} contentContainerStyle={styles.screen}>
      <Pressable accessibilityRole="button" accessibilityLabel={`${friendlyInsight(profile)}. 취향 자세히 보기`} onPress={() => router.push('/taste-profile')} style={styles.hero}>
        <View style={styles.heroIcon}><Icon name="sparkles" size={32} color={colors.espresso} /></View>{profileName ? <Text variant="title3">{profileName}</Text> : null}<Text variant="caption" color={colors.cocoa}>지금 보이는 취향</Text><Text variant="title1">{friendlyInsight(profile)}</Text><Text color={colors.neutral800}>{profile.ratedCupCount < 3 ? `맛을 남긴 커피가 ${3 - profile.ratedCupCount}잔 더 필요해요.` : `${profile.ratedCupCount}잔을 바탕으로 찾았어요.`}</Text><View style={styles.more}><Text variant="label">자세히 보기</Text><Icon name="chevron.right" size={16} /></View>
      </Pressable>
      ...
      <View style={styles.menu}><MenuRow icon="gearshape.fill" title="앱 설정" body="진동, 소리, 화면 움직임, 저장 및 앱 정보" onPress={() => router.push('/settings')} /></View>
    </Screen>
  );
}
