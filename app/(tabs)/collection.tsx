...
import { BeanSummary, CupSummary, RecipeSummary } from '@/components/data';
import { Button, EmptyState, Icon, IconButton, Screen, Text } from '@/components/ui';
...
type Section = 'beans' | 'recipes' | 'gear' | 'cafes';
...
export default function CollectionScreen() {
  ...
  const [cafes, setCafes] = useState<Cup[]>([]);
  const load = useCallback(async () => { const [b, r, g, c] = await Promise.all([listBeans(db, true), listRecipes(db), listUserGear(db), listCups(db, { kind: 'cafe' })]); setBeans(b); setRecipes(r); setGear(g); setCafes(c); }, [db]);
  ...
  return <Screen contentContainerStyle={styles.screen}>
    ...
    {section === 'cafes' && (cafes.length ? <View style={styles.list}>{cafes.map((cup) => <Pressable key={cup.id} accessibilityRole="button" accessibilityLabel={`${cup.cafeName || '카페'}에서 마신 ${cup.beanName} 기록 보기`} onPress={() => router.push(`/cup/${cup.id}`)}><CupSummary cup={cup} /></Pressable>)}</View> : <EmptyState title="저장한 카페 기록이 없어요" body="밖에서 마신 커피도 한 잔의 경험으로 남겨보세요." icon="cup.and.saucer.fill" action={<Button label="카페 커피 기록" onPress={() => router.push('/record-cafe')} />} />)}
  </Screen>;
}
