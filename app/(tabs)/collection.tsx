...
export default function CollectionScreen() {
  ...
  return <Screen header={<View style={styles.header}><Text variant="headerTitle" accessibilityRole="header">보관함</Text></View>} contentContainerStyle={styles.screen}>
...
function gearIcon(value: Gear['category']): Parameters<typeof Icon>[0]['name'] { return ({ grinder: 'dial.medium', dripper: 'cup.and.heat.waves.fill', filter: 'line.3.horizontal.decrease', kettle: 'drop.fill', scale: 'dial.medium', water: 'drop.fill' } as const)[value]; }
const styles = StyleSheet.create({ screen: { gap: spacing.section }, header: { minHeight: 48, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, segment: { flexDirection: 'row', padding: 4, backgroundColor: colors.creamDeep, borderRadius: radius.medium }, segmentItem: { flex: 1, minHeight: 42, alignItems: 'center', justifyContent: 'center', borderRadius: radius.small }, segmentSelected: { backgroundColor: colors.espresso }, segmentTextDefault: { fontFamily: fonts.regular }, list: { gap: spacing.small }, archivedSection: { gap: spacing.small, paddingTop: spacing.small }, archivedRow: { minHeight: 82, flexDirection: 'row', alignItems: 'center', gap: spacing.small, padding: spacing.small, borderRadius: radius.large, backgroundColor: colors.white }, gearList: { backgroundColor: colors.white, borderRadius: radius.large, overflow: 'hidden' }, gear: { minHeight: 82, flexDirection: 'row', alignItems: 'center', gap: spacing.small, padding: spacing.small }, gearIcon: { width: 52, height: 52, borderRadius: 18, backgroundColor: colors.creamDeep, alignItems: 'center', justifyContent: 'center' }, copy: { flex: 1 } });
