...
export default function JournalScreen() {
  ...
  return (
    <Screen contentContainerStyle={styles.screen} footer={compareMode ? <BottomActionBar primaryLabel={selectedCupIds.length === 2 ? '선택한 2잔 비교하기' : `2잔 중 ${selectedCupIds.length}잔 선택`} primaryDisabled={selectedCupIds.length !== 2} onPrimaryPress={() => router.push(`/compare?cupIds=${selectedCupIds.join(',')}`)} /> : undefined}>
      <View style={styles.header}><View style={styles.headerCopy}><Text variant="title1" accessibilityRole="header">{compareMode ? '기록 비교' : '기록'}</Text><Text color={colors.neutral800}>{compareMode ? '같은 원두로 내린 집 브루 2잔을 골라주세요.' : cups.length ? `${cups.length}잔의 커피가 쌓였어요.` : '마신 커피를 차곡차곡 모아보세요.'}</Text></View>{compareMode ? <Button label="선택 취소" variant="tertiary" onPress={cancelCompare} /> : compareAvailable ? <Button label="비교할 기록 고르기" variant="tertiary" onPress={startCompare} /> : null}</View>
