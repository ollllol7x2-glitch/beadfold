...
import { BottomActionBar, Button, Card, EmptySection, EmptyState, Icon, IconButton, PageHeader, Screen, Text, type SymbolName } from '@/components/ui';
  ...
  return (
    <Screen header={<PageHeader title={bean.name} backLabel="보관함" backHref="/(tabs)/collection" action={<IconButton name="ellipsis" label="원두 메뉴 열기" onPress={() => setMenuOpen(true)} />} />} footer={<BottomActionBar primaryLabel="추천대로 내리기" onPrimaryPress={() => router.push(`/recipe/guided?beanId=${bean.id}`)} secondaryLabel="직접 레시피" onSecondaryPress={() => router.push(`/recipe/manual?beanId=${bean.id}`)} />}>
      ...
      <View style={styles.sectionGroup}>
        ...
        {recipes.length ? recipes.map((recipe) => <RecipeSummary key={recipe.id} recipe={recipe} />) : <EmptySection body="아직 저장한 레시피가 없어요." />}
      </View>
      <View style={styles.sectionGroup}>
        ...
        {cups.length ? cups.slice(0, 4).map((cup) => <CupSummary key={cup.id} cup={cup} />) : <EmptySection body="첫 브루잉을 완료하면 경험이 여기에 남아요." />}
      </View>
...
function InventoryHistory({ cups, events }: { cups: Cup[]; events: InventoryEvent[] }) {
  ...
  if (!entries.length) return <EmptySection body="추출을 완료하거나 재고를 맞추면 여기에 남아요." />;
...
const styles = StyleSheet.create({ hero: { backgroundColor: colors.warmBeige }, flex: { flex: 1 }, detail: { flexDirection: 'row', gap: spacing.default, paddingVertical: spacing.compact }, detailDivided: { borderBottomWidth: 1, borderBottomColor: colors.neutral200 }, label: { width: 104 }, sectionGroup: { gap: spacing.small }, section: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: spacing.small }, sectionButton: { minHeight: 36, minWidth: 0, paddingHorizontal: 12, paddingVertical: 7 }, inventory: { gap: spacing.compact, marginTop: spacing.compact, padding: spacing.small, borderRadius: radius.medium, backgroundColor: colors.white }, inventoryHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: spacing.small }, inventoryResult: { gap: 2 }, history: { gap: 0 }, historyRow: { minHeight: 62, flexDirection: 'row', alignItems: 'center', gap: spacing.small, paddingVertical: spacing.compact }, historyRowDivided: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.neutral200 }, historyAmount: { alignItems: 'flex-end', gap: 2 }, menuLayer: { flex: 1 }, menu: { position: 'absolute', top: 76, right: 18, width: 224, overflow: 'hidden', borderRadius: radius.large, backgroundColor: colors.white, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.neutral200, shadowColor: '#000', shadowOpacity: 0.16, shadowRadius: 18, shadowOffset: { width: 0, height: 8 }, elevation: 8 }, menuAction: { minHeight: 52, flexDirection: 'row', alignItems: 'center', gap: spacing.small, paddingHorizontal: spacing.small }, pressed: { opacity: 0.62 } });
