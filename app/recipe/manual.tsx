...
import { BottomActionBar, BottomSheet, Button, Card, Chip, Field, goBackOrReplace, Icon, InfoNote, Screen, TaskHeader, Text } from '@/components/ui';
...
export default function ManualRecipeScreen() {
  ...
  const updateText = (key: keyof Pick<Recipe, 'name' | 'grindTarget' | 'grinder' | 'dripper' | 'filter' | 'waterProfile'>, value: string) => {
    if (!recipe) return;
    setErrors([]);
    setRecipe({ ...recipe, [key]: value, updatedAt: new Date().toISOString() });
  };
  ...
  const doseError = errors.find((error) => error.startsWith('원두량'));
  const waterError = errors.find((error) => error.startsWith('물양'));
  const temperatureError = errors.find((error) => error.startsWith('물 온도'));
  const totalTimeError = errors.find((error) => error.startsWith('총 시간'));
  const formErrors = errors.filter((error) => ![doseError, waterError, temperatureError, totalTimeError].includes(error));
  ...
  const focusFirstInvalid = () => {
    if (doseError || validation.some((error) => error.startsWith('원두량'))) return focusField('dose');
    if (waterError || validation.some((error) => error.startsWith('물양'))) return focusField('water');
    if (temperatureError || validation.some((error) => error.startsWith('물 온도'))) return focusField('temperature');
    if (totalTimeError || validation.some((error) => error.startsWith('총 시간'))) return focusField('totalTime');
  };
  const save = async () => { if (!recipe) return; if (validation.length) { setErrors(validation); focusFirstInvalid(); return; } await saveRecipe(db, recipe); setErrors([]); setBaseline(JSON.stringify(recipe)); setSaved(true); };
  const brew = async () => { if (!recipe || !bean) return; if (validation.length) { setErrors(validation); focusFirstInvalid(); return; } await saveRecipe(db, recipe); allowExit(); const session = await startBrew(db, bean, recipe); router.replace(`/brew/${session.id}`); };
  ...
  return (
    <Screen showNavigation={false} header={<TaskHeader title="내 방식으로 내리기" onClose={() => requestExit(() => goBackOrReplace(`/bean/${bean.id}`))} />} footer={<BottomActionBar primaryLabel="이 레시피로 브루잉" primaryDisabled={Boolean(validation.length)} onPrimaryPress={() => void brew()} secondaryLabel="저장" onSecondaryPress={() => void save()} />}>
      {formErrors.length ? <Card style={styles.error}>{formErrors.map((error) => <Text key={error} accessibilityRole="alert" color={colors.error}>오류: {error}</Text>)}</Card> : null}
      {sourceCupName ? <Card tone="tinted"><Text variant="label">더 좋았던 기록을 가져왔어요</Text><InfoNote body={`${sourceCupName}의 추출값을 시작점으로, 필요하면 조금만 조절해보세요.`} /></Card> : null}
      {saved ? <Text accessibilityRole="alert" color={colors.neutral600}>저장했어요. 다음에도 이 레시피를 사용할 수 있어요.</Text> : null}
